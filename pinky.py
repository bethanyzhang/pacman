...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        # START CODE CHANGES
        if on_vert :
            # check picky's positon and pacman's postion: 
            if up_down_part>0 :
                self.x_add = 0
                self.y_add  = self.velocity
            elif abs(up_down_part) == 0:
                self.y_add = 0
            else:
                self.y_add = -self.velocity
            ...
            if on_horz:
                if abs(up_down_part)>abs(left_right_part):
                    self.x_add = 0
                    self.y_add = self.velocity if up_down_part>0 else -self.velocity
                else:
                    self.y_add = 0
                    self.x_add = self.velocity if left_right_part>0 else -self.velocity
